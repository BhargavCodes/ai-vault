...
const Dashboard = () => {
  ...
  const runAI = async (fileId) => {
    ...
    try {
      ...
      const updatedFile = res.data.file; // Backend returns the fresh file object

      // 1. Update the list immediately
      setFiles(prevFiles => prevFiles.map(f => f.id === fileId ? updatedFile : f));

      // 2. If the modal is open for this file, update it too
      ...
      toast.error(err.response?.data?.error || "AI Analysis Failed", { id: loadingToast });
    } finally {
      setAnalyzing(null);
    }
  };
  ...
  const handleChatSubmit = async (e) => {
    ...
    try {
        const res = await api.post(`/files/${selectedFile.id}/chat`, { question });
        setChatHistory(prev => [...prev, { role: 'ai', text: res.data.answer }]);
    } catch (err) {
        setChatHistory(prev => [...prev, { role: 'ai', text: "Error: Failed to get answer." }]); 
    } finally {
  ...
  const suggestName = async () => {
    if (!selectedFile.is_analyzed) { toast.error("Please run AI Analysis first!"); return; }
    ...
    } catch (err) { toast.error(err.response?.data?.error || "Failed to suggest name"); } finally { setRenamingLoading(false); }
  };
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100 dark:bg-gray-800 dark:border-gray-700">
          <div className="relative w-full md:w-96 group">
              ...
              <input type="text" placeholder="Search files, tags, content..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-all dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
          </div>
      ...
      {filteredFiles.length === 0 ? (
          <div {...getRootProps()} className={`flex flex-col items-center justify-center py-32 text-gray-400 border-2 border-dashed rounded-3xl transition-colors cursor-pointer ${isDragActive ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 bg-gray-50/50 hover:border-blue-300 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800/50 dark:hover:bg-gray-800'}`}>
              <input {...getInputProps()} disabled={uploading} />
              <div className="bg-white p-4 rounded-full shadow-sm mb-4 dark:bg-gray-700">
                  <UploadCloud size={48} className={isDragActive ? "text-blue-500" : "text-blue-100 dark:text-blue-400"} />
              </div>
              <h3 className="text-lg font-medium text-gray-600 dark:text-gray-300">{isDragActive ? "Drop the file now!" : "No files found"}</h3>
              <p className="text-sm dark:text-gray-500">{isDragActive ? "Release to upload" : "Upload or Drag & Drop a file here."}</p>
          </div>
      ) : (
                  ...
                  <div key={file.id} onClick={() => setSelectedFile(file)} className="group bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col h-full cursor-pointer dark:bg-gray-800 dark:border-gray-700">
                      ...
                      <div className="p-5 flex-1 flex flex-col">
                          <div className="flex justify-between items-start mb-3">
                              <h3 className="font-bold text-gray-800 truncate flex-1 pr-2 dark:text-gray-200">{file.filename}</h3>
                          ...
                          <div className="flex-1">
                              {file.is_analyzed ? (
                                  <div className="space-y-3 animate-in fade-in duration-500">
                                      ...
                                      {file.summary && (
                                          <p className="text-xs text-gray-500 line-clamp-3 bg-gray-50 p-2 rounded border border-gray-100 dark:bg-gray-700/50 dark:border-gray-600 dark:text-gray-400 mt-1">
                                              {file.summary.replace(/\*/g, '').substring(0, 150)}...
                                          </p>
                                      )}
                                  </div>
                              ) : (
      ...
      {/* ✨ ADVANCED MODAL (Fixed Z-Index and Height) */}
      {selectedFile && (
                        ...
                        {activeTab === 'details' ? (
                            !selectedFile.is_analyzed ? (
                                ...
                                <div className="space-y-6 animate-in fade-in">
                                    ...
                                    <div className="pt-6 mt-6 border-t border-gray-100 dark:border-gray-700 flex justify-end"><button onClick={() => runAI(selectedFile.id)} disabled={analyzing === selectedFile.id} className="text-xs text-gray-400 hover:text-blue-600 flex items-center gap-1.5 transition hover:underline"><RotateCcw size={12} className={analyzing === selectedFile.id ? "animate-spin" : ""} /> Re-run Analysis</button></div>
                                </div>
                            )
                        ) : (
